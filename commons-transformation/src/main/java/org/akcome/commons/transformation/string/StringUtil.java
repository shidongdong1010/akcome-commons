package org.akcome.commons.transformation.string;
...
public class StringUtil {
	 private static final int INDEX_NOT_FOUND = -1;  
		...
		public static byte uniteBytes(byte src0, byte src1) {
			...
			byte _b1 = Byte.decode("0x" + new String(new byte[] { src1 })).byteValue();// 不左移，保持在低4位
	     ...
	     */    
	    public static String encode(String chinese, String charset) throws UnsupportedEncodingException{    
	        chinese = (chinese==null ? "" : chinese);    
	        String  enChinese  =  chinese;
	        try {    
	        	enChinese =  URLEncoder.encode(chinese, charset);    
	        } catch (UnsupportedEncodingException e) {    
	        	enChinese  =  chinese;
	        	throw  new  UnsupportedEncodingException("编码字符串[" + chinese + "]时发生异常:系统不支持该字符集[" + charset + "]");
	        }    
	        return enChinese;    
	    }    
	     ...
	     */    
	    public static String decode(String chinese) throws UnsupportedEncodingException{    
	     ...
	     */    
	    public static String decode(String chinese, String charset) throws UnsupportedEncodingException{    
	        chinese = (chinese==null ? "" : chinese); 
	        String  deChinese  =  chinese;
	        try {    
	        	deChinese =  URLDecoder.decode(chinese, charset);    
	        } catch (UnsupportedEncodingException e) {    
	        	deChinese  =  chinese;
	        	throw  new  UnsupportedEncodingException("解码字符串[" + chinese + "]时发生异常:系统不支持该字符集[" + charset + "]");    
	        }    
	        return deChinese;    
	    }    
}
